...
############################################
setwd("E:/Dropbox/PMA6/PMA6_Shared/latex/Chapters/Chapter04/figures")
data.path <- "../../../../data6e/"
  
# Load data & do some minor data management 
...
parhiv <- raw %>% mutate(JOBMO = factor(JOBMO, labels=c("Employed", "Unemployed", "Retired/Disabled")), 
                         ...
                         bsi_overall =rowMeans(raw[,grep("BSI", names(raw))]), log_bsi_overall = log(bsi_overall+.01), 
...
depress <- raw.d %>% mutate(gender = factor(SEX, labels=c("Male", "Female")), 
                            educat = factor(EDUCAT, levels=c("<HS", "Some HS", "HS Grad", "Some college", "BS", "MS", "PhD")), 
...
####-- demo3 --####
pdf("demo3_a.pdf", width=5, height=5)
hist(parhiv$bsi_overall);box()
dev.off()


demo3_b <- ggplot(parhiv, aes(x=bsi_overall)) + geom_histogram() +theme_bw(base_size=16)
ggsave("demo3_b.pdf", width=5, height=5)

demo3_c <- ggplot(parhiv, aes(x=bsi_overall)) + geom_histogram() + theme_classic(base_size = 20) + 
            xlab("BSI score") + ylab("Frequency") + ggtitle("Brief Symptom Inventory Score")
ggsave("demo3_c.pdf", width=6, height=5)


####-- univ_bar_n --####
univ_bar_n <- ggplot(filter(parhiv, !is.na(EDUMO)), aes(x=EDUMO)) + theme_classic(base_size = 20) + 
...
ggplot(depress, aes(y=AGE, x=1)) + geom_jitter(position = position_jitter(0.2)) + ylab("Age") + 
  scale_x_continuous(limits=c(0,2)) +  theme_classic(base_size=20)+ xlab("")+
...
ggsave("univ_stripchart.pdf", width=7)


####-- univ_hist --####
univ_hist <- ggplot(depress, aes(x=AGE)) + 
  geom_histogram(colour="black", fill="grey80", binwidth=10) +  xlab("Age") + ylab("Frequency") + 
  ggtitle("Distribution of age") + theme_classic(base_size = 16) +
  scale_x_continuous(limits=c(20,80), breaks=seq(20,80,by=10))

ggsave("univ_hist.pdf")


####-- univ_hist_defaultbins --####
univ_hist_defaultbins <- ggplot(depress, aes(x=AGE)) + 
  geom_histogram(colour="black", fill="grey80") +  xlab("Age") + ylab("Frequency") + 
  ggtitle("Distribution of age") + theme_classic(base_size = 16) +
  scale_x_continuous(limits=c(15,90), breaks=seq(20,90,by=10))

ggsave("univ_hist_defaultbins.pdf")

####-- univ_stripchart--####
# ggplot(depress, aes(x=cesd, y=1)) + geom_point()
...
ggplot(depress, aes(x=AGE)) + 
  geom_histogram(aes(y=..density..), colour="black", fill="grey80") +  xlab("Age") + ylab("Density") + 
  geom_density(colour="black", lwd=1.1) + 
  scale_x_continuous(limits=c(15,90), breaks=seq(20,90,by=10)) + 
  ggtitle("Distribution of age") + theme_classic(base_size = 16)
ggsave("univ_density.pdf", width=5, height=5)

####-- univ_boxplot --####

txt <- boxplot(depress$AGE, plot=FALSE)$stats
data.labels <- data.frame(value=as.vector(txt), lab=as.character(txt))
labels <- data.frame(value=as.vector(txt), lab=c("Min", "Q(0.25)", "Median", "Q(0.75)", "Max"))
...
ggsave("univ_boxplot.pdf", height=7, width=5)

####-- univ_boxplot_modified --####
txt <- boxplot(depress$CESD, plot=FALSE)$stats
data.labels <- data.frame(value=as.vector(txt), lab=as.character(txt))
labels <- data.frame(value=as.vector(txt), lab=c("Min", "Q(0.25)", "Median", "Q(0.75)", "Largest value \n within fence"))
...
ggsave("univ_boxplot_modified.pdf", height=7, width=5)
...
ggsave("univ_boxplot_enhanced_a.pdf", height = 5, width=5)
...
ggplot(depress, aes(y=CESD, x=1)) + 
  ...
  geom_jitter(alpha=.3, width=.1) +
  theme(axis.ticks = element_blank(), axis.text.x = element_blank()) 
ggsave("univ_boxplot_enhanced_b.pdf", height = 5, width=5)


####-- two-way freq --####
depress %$% table(gender, educat) %>% xtable()
...
ggplot(phna, aes(x=EDUMO, fill=JOBMO)) + geom_bar() + theme_classic(base_size = 16) + 
...
ggsave("biv_bar_stacked.pdf", width=5)


####-- biv_bar_stacked_pct --####
ggplot(pct.cc, aes(x=EDUMO, fill=JOBMO)) + geom_bar(aes(y=pct), stat='identity') + 
  scale_fill_grey(guide=FALSE) + theme_classic(base_size = 16) + xlab("") + ylab("Percent") + 
...
ggsave("biv_bar_stacked_pct.pdf", width=5)
  

####-- biv_bar_side --####
ggplot(pct.cc, aes(x=EDUMO, y=pct, fill=JOBMO)) + scale_fill_grey(name="Job Status") + 
  geom_bar(stat="identity", position = "dodge") +  theme_classic(base_size = 16) + xlab("") + 
  scale_y_continuous(labels = percent) + ylab("Percent") + 
  scale_x_discrete(labels = function(x) lapply(strwrap(x, width = 12, simplify = FALSE), 
                                               paste, collapse="\n")) + 
  theme(axis.ticks.x=element_blank()) 

ggsave("biv_bar_side.pdf", width=7, height=4.5)
...
ggsave("biv_dot_plot.pdf", width=7, height=5)


####-- bif dot plot differences --####
...
ggplot(bdpd, aes(x=count, y=reorder(JOBMO, count))) + 
        ...
        xlab("Frequency") + 
        ylab("Mother's Job Status") + theme_bw(base_size = 16) +
        theme(panel.grid.major.y = element_blank())  + 
        theme(axis.ticks.y=element_blank()) 

ggsave("biv_dot_plot_diff.pdf", width=7, height=4)
...
pdf("mosaic_plot.pdf", height=6, width=6)
...
ggsave("biv_scatter.pdf", width=7, height=5)

####-- profile --####
ggplot(mice, aes(y=WEIGHT, x=DAY, group=ID)) + geom_point() + geom_line() + 
  theme_classic(base_size = 16) + ylab("Weight (g)") + xlab("Time (Days)")
ggsave("profile_plot.pdf", width=7, height=5)
...
ggsave("biv_cont_cat_a.pdf", height=6, width=6)

ggplot(depress, aes(x=CESD, color=gender)) + 
  geom_density() + theme_classic(base_size = 20) + ylab("Density") + xlab("CESD")+
  scale_color_grey(start=0, end=0.5)+
  theme(axis.ticks = element_blank(), axis.text.y = element_blank()) + 
  theme(legend.justification=c(1,1), legend.position=c(1,1))

ggsave("biv_cont_cat_b.pdf", height=6, width=6)

ggplot(depress, aes(x=CESD, fill=gender)) +  facet_grid(gender ~ .) + xlab("CESD")+
  geom_histogram() + theme_bw(base_size = 20) + ylab("Frequency") + 
  scale_fill_grey(start=0, end=0.5,guide=FALSE) +
  theme(axis.ticks = element_blank(), axis.text.y = element_blank()) + 
  theme(legend.justification=c(1,1), legend.position=c(1,1))

ggsave("biv_cont_cat_c.pdf", height=6, width=6)

####-- scatter3d --####
ggplot(depress, aes(x=AGE, y=CESD, color=gender)) + scale_color_grey() + 
  geom_point() +  theme_classic(base_size = 20) + xlab("Age") + ylab("CESD")
ggsave("scatter_3d_a.pdf", height=4.5, width=5)

ggplot(depress, aes(x=AGE, y=CESD, shape=gender)) + geom_point() +  theme_classic(base_size = 20) + 
  xlab("Age") + ylab("CESD")
ggsave("scatter_3d_b.pdf", height=4.5, width=5)

ggplot(depress, aes(x=AGE, y=CESD, size=INCOME)) + 
  geom_point(shape=21) + theme_classic(base_size = 20) + xlab("Age") + ylab("CESD") + 
  scale_size_continuous(limits=c(0,70), breaks = seq(0,70,by=10))
ggsave("scatter_3d_c.pdf", height=4.5, width=5)

ggplot(depress, aes(x=AGE, y=CESD, fill=INCOME)) + 
  geom_point(shape=21, size=5) +
  theme_classic(base_size = 20) + xlab("Age") + ylab("CESD") + 
  scale_fill_gradient(low="white", high="black")
ggsave("scatter_3d_d.pdf", height=4.5, width=5)


####-- hist_panel --####

ggplot(filter(parhiv, !is.na(EDUMO) & !is.na(JOBMO)), aes(x=bsi_overall)) +  ylab("Frequency") + 
  geom_histogram()+  facet_grid(EDUMO~JOBMO, labeller=label_wrap_gen(14)) +
  scale_y_continuous(breaks=seq(0,10,by=2))+
  theme_bw(base_size = 18) +xlab("Overall BSI score")
ggsave("hist_panel.pdf", width=8, height=6)
